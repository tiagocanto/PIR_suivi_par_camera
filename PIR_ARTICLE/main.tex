...
The world of computer vision developers is growing and one of the biggest communities is the Roboflow Universe [??? Citation 1???], which we can find a great number of models and datasets. The project Rugby Detection Computer Vision Model  [!!! Citation 2 !!!]  was the closest to our project’s needs, and the pictures used in this dataset are similar to the images we captured in an INSA AS match. 

After testing the model from the project (RDCVM) [!!! Citation 2 !!!] in some images from the captured match, positive results were achieved, but it had some problems confusing the arbitrator with a player, not identifying all the players, and sometimes not seeing the ball.  

POSITiVE results will be here

Unfortunately, the model [!!! Citation 2 !!!] isn’t accessible to download, so it wouldn’t  be possible to use in a real time application, because for every frame the Roboflow repository would have to be accessed on the internet, increasing the cost and delay of the image processing. 

To solve the presented problem, the project’s dataset [!!! Citation 2 !!!] was cloned and a new AI model was trained, hoping to achieve the same results as the previous. Four models were created, two with yolov8n, one with yolov8s and the last with yolov8m; they were all tested in an image not from the dataset [??? Show images of the test??? ].  
...
To improve this project, we plan on using another YOLO model to see the statistics of the game as well, identifying tackle, penalty kick, throw-in, scrum, and other similar rugby situations that our other model can’t identify. Another aimed improvement is to create a system so the model could be able to after identifying the players, separate them between team A and team B. The system consists of a Sigmoid loss language image pre-trained[!!! Citation!!!] (extract color information from the player’s shirt), associated with an UMAP[!!! Citation !!!] (compact the previous information) and a Kmeans [!!! Citation !!!] (sort both teams). These improvements would highly increase the quality of the performance analysis of the teams. 
...
\subsection{Video Transmission teams part}
