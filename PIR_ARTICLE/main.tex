...
%%%%%%%%%%%%%%%%%%%% DZLL %%%%%%%%
A model of Artificial Inteligence is a trained code to learn paterns based on a database[citar qualquer coisa que defina um modelo]. In between the existent models, there is the model YOLO whose algorithm treats the image only one time (You Only Look Once).  

Models like YOLO have faster responses to the video processing, so they are usually used for real time processing, autonomous car, monitoring, etc.  
The detection of plenty of objects at the same time is also a caracteristc of these models, but the main reason we chose YOLO as our model was to process efficiently and high speed.
...
The Figure\ref{fig:ultralytics_models_comparison}\cite{ultralytics_models} compares the different versions available of YOLO’s models by latency and precision of detection. 
Based on these models, considering the speed as main goal to track in real time during a rugby match, we chose to work with YOLOv8 version.
\begin{figure}[H]
    \centering
...
\end{figure}
Ultralytics website also presents the performance metrics of different models of the chosen version as shown at the Figure~\ref{fig:yolov8_performance}\cite{ultralytics_models}. 
Tests were made to choose best way to process images, having a good balance between accuracy and processing time.%% !t ? to no leave space in the text and continue the text?
\begin{figure}[H]
    \centering
